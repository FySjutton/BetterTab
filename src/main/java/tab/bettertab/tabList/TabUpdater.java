...
public class TabUpdater {
    ...
    public static List<FormattedCharSequence> headerList;
    public static List<FormattedCharSequence> footerList;
    public static int footerStartY;

    public static int pageNumber;

    public static void update(Minecraft client, List<PlayerInfo> playerEntries, Component header, Component footer, Scoreboard scoreboard, Objective objective) {
        ...
        if (!headerText.equals(Component.empty())) {
            headerList = client.font.split(headerText, maxScreenWidth);
        } else {
            headerList = new ArrayList<>();
        }
        int headerHeight = 5 + headerList.size() * client.font.lineHeight;
        if (BetterTabConfig.CONFIG.instance().renderFooter && footer != null) {
            footerList = client.font.split(footer, maxScreenWidth);
        } else {
            footerList = new ArrayList<>();
        }
        int footerHeight = footerList.size() * client.font.lineHeight + 5;
...
// Calculate which columns are to be displayed
        ...
        int serverInfoWidth = Math.max(
                headerList.isEmpty() ? 0 : Collections.max(headerList.stream().map(client.font::width).toList()),
                footerList.isEmpty() ? 0 : Collections.max(footerList.stream().map(client.font::width).toList())
        ) + 10;
